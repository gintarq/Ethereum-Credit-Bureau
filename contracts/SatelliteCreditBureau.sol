...
contract SatelliteCreditBureau is CCIPReceiver {
        ...
        returns (uint256)
    {
        Client.EVM2AnyMessage memory message = Client.EVM2AnyMessage({
            ...
            data: payload,
            tokenAmounts: new Client.EVMTokenAmount[](0), // Assuming no token amounts for now
        ...
        return IRouterClient(this.getRouter()).getFee(destinationChainId, message);
    }
        ...
        internal 
    {
        ...
        IRouterClient(this.getRouter()).ccipSend(destinationChainId, message);
    }
    ...
    // Decoding the received data into a report
    (ICreditBureau.Report memory report, address user) = abi.decode(message.data, (ICreditBureau.Report, address));
    
    // Handle the report: for instance, save it, emit an event, etc.
    // For this example, I'll just emit an event:
